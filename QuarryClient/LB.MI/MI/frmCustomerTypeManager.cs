...
namespace LB.MI
{
    public partial class frmCustomerTypeManager : LBUIPageBase
    {
        public frmCustomerTypeManager()
        {
            InitializeComponent();
        }
        ...
        {
            DataTable dtDesc = ExecuteSQL.CallView(139, "", "", "");
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                if (LB.WinFunction.LBCommonHelper.ConfirmMessage("是否删除选中行？", "提示", MessageBoxButtons.YesNo) == DialogResult.Yes)
                {
                    ...
                    dtSPIN.Columns.Add("CustomerTypeID", typeof(long));
                    ...
                    foreach (DataGridViewRow dgvr in this.grdMain.SelectedRows)
                    {
                        ...
                        long lCustomerTypeID = LBConverter.ToInt64(drv["CustomerTypeID"]);

                        if (lCustomerTypeID > 0)
                        {
                            DataRow drNew = dtSPIN.NewRow();
                            drNew["CustomerTypeID"] = lCustomerTypeID;
                    ...
                    DataSet dsReturn;
                    DataTable dtReturn;
                    ExecuteSQL.CallSP(14802, dtSPIN, out dsReturn, out dtReturn);
