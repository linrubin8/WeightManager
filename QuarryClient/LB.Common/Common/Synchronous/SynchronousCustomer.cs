...
namespace LB.Common.Synchronous
{
    public class SynchronousCustomer
    {
        ...
        public static void CompareClientDiffServer(out DataTable dtAddCustomer)
        {
            //先读取服务器客户资料
            DataTable dtServer = ExecuteSQL.CallView_Service(112, "CustomerName", "", "CustomerID");
            //读取本地客户资料
            DataTable dtClient = ExecuteSQL.CallView(112);
            ...
            foreach (KeyValuePair<string, DataRow> kvClient in dictClient)
            {
                ...
                {
                    dtAddCustomer.ImportRow(kvClient.Value);
                }
            }
        }
